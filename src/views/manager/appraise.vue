    ...
    <el-table :data="list">
      <el-table-column prop="orderId" label="订单编号" width="300">
        <template #default="{row}">
          000000000{{row.orderId}}
        </template>
      </el-table-column>
      <el-table-column prop="bikeName" label="电动车名称" width="260"/>
      <el-table-column prop="userName" label="用户名称" width="260"/>
      <el-table-column prop="appraiseStar" label="评价星级"/>
      <el-table-column prop="appraiseDescription" label="评价描述"/>
    </el-table>
    <el-pagination
...
export default {
  ...
  methods: {
    queryPage() {
      commonApi.getAppraises(this.pageInfo).then((res) => {
        if (res.code === 200) {
